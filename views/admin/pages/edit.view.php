<div class="mx-auto max-w-xl py-8">
    <h2 class="mb-8 font-display font-bold text-accent" style="font-size: var(--fluid-lg);">Update This Post</h2>

    <form action="index.php?<?= route_url(['route' => 'admin/pages/edit', 'id' => $pageData->id]) ?>" method="post" id="input-form"
        ...
        <div class="flex flex-col gap-1">
            ...
            <input type="text" name="name" id="input-name" value="<?= e($pageData->name) ?>"
        ...
        <div class="flex flex-col gap-1">
            <label for="slug-update" class="text-sm uppercase tracking-wider text-ink/70">Slug</label>
            <div class="flex items-center gap-3">
                <input type="text" name="slug" id="slug-update" value="<?= e($pageData->slug) ?>" disabled
                       class="w-full border border-line bg-surface px-3 py-2 text-ink outline-none transition-colors duration-300 ease-out focus:border-accent disabled:opacity-50">
                <label class="flex items-center gap-2 text-sm text-ink/70 whitespace-nowrap">
                    <input type="checkbox" id="slug-update-checkbox" class="accent-[var(--color-accent)]">
                    Edit
                </label>
            </div>
        </div>
        ...
        <div class="flex flex-col gap-1">
            ...
            <input type="datetime-local" name="date_published" id="date_published" value="<?= e($pageData->date_published) ?>"
            ...
            <label for="image"
                ...
                <?php if (!empty($pageData->image_url)): ?>
                    <img id="image-preview"
                         src="<?= e(image_get_size($pageData->image_url, 'small')) ?>"
                         alt="<?= e($pageData->name) ?>"
                         class="block w-full h-full aspect-video object-cover">
                <?php else: ?>
                    <img id="image-preview"
                         src=""
                         alt=""
                ...
                <div id="image-placeholder"
                     class="<?= empty($pageData->image_url) ? 'flex' : 'hidden' ?> aspect-video w-full items-center justify-center text-sm uppercase tracking-wider text-ink/50">
                ...
                <div class="pointer-events-none absolute inset-0 flex items-center justify-center bg-ink/0 text-[var(--color-on-accent)] opacity-0 transition-all duration-300 ease-out group-hover:bg-ink/40 group-hover:opacity-100">
                    <span class="text-sm uppercase tracking-wider">Change image</span>
                </div>
        ...
        <div class="flex flex-col gap-1">
            ...
            <textarea name="text" id="text" rows="10"
                      class="w-full border border-line bg-surface px-3 py-2 text-ink outline-none transition-colors duration-300 ease-out focus:border-accent"><?= e($pageData->text) ?></textarea>
        </div>
